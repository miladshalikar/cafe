...
func (s Service) UploadMedia(ctx context.Context, req mediaparam.UploadMediaRequest) (mediaparam.UploadMediaResponse, error) {
	const op = "mediaservice.UploadMedia"

	fileName := strconv.FormatInt(time.Now().UnixNano(), 10) + filepath.Ext(req.Filename)

	file, fErr := req.Open()
	if fErr != nil {
		return mediaparam.UploadMediaResponse{}, richerror.New(op).
			WithWarpError(fErr).
	...
	if iErr != nil {
		return mediaparam.UploadMediaResponse{}, richerror.New(op).WithWarpError(iErr)
	}

	res, gErr := s.GetURLMedia(ctx, mediaparam.GetURLRequest{ID: insertedMedia.ID})
	if gErr != nil {
		return mediaparam.UploadMediaResponse{}, richerror.New(op).WithWarpError(gErr)
	}
